      ...
      new Manifest(
        {
          ...
          metadata: new Metadata({ title: new LocalizedString("Title") }),
          links: new Links([new Link({ href: "/manifest.json", rels: new Set(["self"]) })]),
          readingOrder: new Links([new Link({ href: "/chap1.html", type: "text/html" })]),
          resources: new Links([new Link({ href: "/image.png", type: "image/png" })]),
          tableOfContents: new Links([new Link({ href: "/cover.html" }), new Link({ href: "/chap1.html" })]),
        }
      )
    ).toEqual(
      Manifest.fromJSON(
