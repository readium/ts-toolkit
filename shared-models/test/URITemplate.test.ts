...
describe('URITemplate Tests', () => {
  ...
  it('parameters works fine', () => {
    expect(
      new URITemplate('/url{?x,hello,y}name{z,y,w}').parameters
    ).toEqual(
  ...
  it('expand works fine with simple string templates', () => {
    expect(
      ...
      '/urlaaa,Hello%2C%20world,bname45,b,w'
    );
  });
  ...
  it('expand works fine with form-style ampersand-separated templates', () => {
    expect(
      ...
      '/url?x=aaa&hello=Hello%2C%20world&y=bname'
    );
  });
  ...
  it('expand works fine with form-style ampersand-separated templates', () => {
    ...
    let urlPart = 'https://lsd-test.edrlab.org/licenses/39ef1ff2-cda2-4219-a26a-d504fbb24c17/renew';
